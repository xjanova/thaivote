    ...
    <AdminLayout>
        <div class="max-w-3xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">แก้ไขการเลือกตั้ง</h1>
                    <p class="text-gray-600">{{ election.name }}</p>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            >
                            <input
                                v-model.number="form.total_votes_cast"
                                type="number"
                                min="0"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            />
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1"
                                >อัตราการมาใช้สิทธิ (%)</label
                            >
                            <input
                                v-model.number="form.voter_turnout"
                                type="number"
                                min="0"
                                max="100"
                                step="0.01"
...
const form = useForm({
    ...
    name_en: props.election.name_en || '',
    type: props.election.type,
    description: props.election.description || '',
    election_date: props.election.election_date?.split('T')[0] || '',
    start_time: props.election.start_time || '',
    end_time: props.election.end_time || '',
    status: props.election.status,
    total_eligible_voters: props.election.total_eligible_voters,
    total_votes_cast: props.election.total_votes_cast,
    voter_turnout: props.election.voter_turnout,
    is_active: props.election.is_active,
});
...
const submit = () => {
    form.put(route('admin.elections.update', props.election.id));
};
</script>
