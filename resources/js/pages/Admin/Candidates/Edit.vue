            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">
                        แก้ไข {{ candidate.title }} {{ candidate.first_name }} {{ candidate.last_name }}
                    </h1>
                    <p class="text-gray-600">แก้ไขข้อมูลผู้สมัครรับเลือกตั้ง</p>
                </div>
            </div>
                ...
                <div class="p-6 border-b border-gray-200">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-24 h-24 rounded-lg overflow-hidden bg-gray-100">
                            <img
                                v-if="photoPreview || candidate.photo"
                                :src="photoPreview || `/storage/${candidate.photo}`"
...
const form = useForm({
    election_id: props.candidate.election_id,
    party_id: props.candidate.party_id || '',
    constituency_id: props.candidate.constituency_id || '',
    ...
    nickname: props.candidate.nickname || '',
    photo: null,
    biography: props.candidate.biography || '',
    birth_date: props.candidate.birth_date || '',
    education: props.candidate.education || '',
    occupation: props.candidate.occupation || '',
    type: props.candidate.type,
    party_list_order: props.candidate.party_list_order,
    is_pm_candidate: props.candidate.is_pm_candidate || false,
});

const selectedProvince = ref(props.candidate.constituency?.province_id || '');
...
const submit = () => {
    form.post(route('admin.candidates.update', props.candidate.id), {
        forceFormData: true,
        _method: 'PUT',
    });
};
